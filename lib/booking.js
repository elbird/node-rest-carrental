...
function updateRelation(booking, callback) {
	...
	if (booking.rentalcar_id) {
		RentalCar.get(booking.rentalcar_id, function (err, car) {
			if (err) {
				callback(new Error("Booking: rentalcar with id '" + booking.rentalcar_id + "' does not exist"));
				return;
			}
			...
			if (car.booking_ids.indexOf(bookingId) === -1) {
				...
				car.save(function (err) {
					if (err) {
						console.log(err);
						callback(new Error("Booking: booking id could not be added to rentalcar with id: '" + booking.rentalcar_id + "'"));
					} else {
						callback();
					}
				});
...
Booking.before('destroy', function (err, booking, callback) {
	"use strict";
	if (booking.rentalcar_id) {
		RentalCar.get(booking.rentalcar_id, function (err, car) {
			if (err) {
			...
			if (index !== -1) {
				...
				RentalCar.save(car, function (err) {
					if (err) {
						callback(new Error("Booking: colud not remove booking id from rentalcar with id '" + booking.rentalcar_id + "'"));
					}
					callback();
				});
			} else {
